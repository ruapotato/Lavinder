...
First, you need to install all of Lavinder's dependencies (although some are
...
Lavinder uses xcffib_ as an XCB binding, which has its own instructions for
...
Lavinder uses cairocffi_ with XCB support via xcffib. You'll need ``libcairo2``,
...
apt-get install libpangocairo-1.0-0``. Lavinder uses this to provide text
rendering (and binds directly to it via cffi with a small in-tree binding).

asyncio/trollius
----------------

Lavinder uses the asyncio module as introduced in `PEP 3156`_ for its event loop.
...
Stable versions of Lavinder can be installed from PyPI:
